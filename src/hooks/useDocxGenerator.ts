...
import { generateDocx, type GenerateOptions } from '../lib/docx/generator';
import type { StyleSettings } from '../types/styles';

interface UseDocxGeneratorResult {
  generate: (markdown: string, styles: StyleSettings, options?: GenerateOptions) => Promise<void>;
...
export function useDocxGenerator(): UseDocxGeneratorResult {
  ...
  const generate = useCallback(
    async (markdown: string, styles: StyleSettings, options: GenerateOptions = {}) => {
      if (!markdown.trim()) {
      ...
      try {
        const blob = await generateDocx(markdown, styles, options);
